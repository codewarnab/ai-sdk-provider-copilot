...
__export(index_exports, {
  CopilotLanguageModel: () => CopilotLanguageModel,
  createAbortError: () => createAbortError,
  createCopilotProvider: () => createCopilotProvider,
  createStreamContext: () => createStreamContext,
  extractLatestUserMessage: () => extractLatestUserMessage,
  getDefaultUsage: () => getDefaultUsage,
  isRetryableError: () => isRetryableError,
  mapCopilotError: () => mapCopilotError,
  mapEventToStreamParts: () => mapEventToStreamParts,
  mapFinishReason: () => mapFinishReason,
  mapPromptToCopilotFormat: () => mapPromptToCopilotFormat,
  mapUsageEvent: () => mapUsageEvent
});
...
var import_copilot_sdk = require("@github/copilot-sdk");
...
function mapCopilotError(error) {
  if (error instanceof Error) {
    ...
    if (message.includes("unauthorized") || message.includes("authentication") || message.includes("api key") || message.includes("credentials") || message.includes("not authenticated") || message.includes("login required")) {
      return new import_provider.LoadAPIKeyError({ message: error.message });
    }
  ...
  switch (event.type) {
    ...
    case "assistant.message_delta": {
      ...
      if (!context.textStarted) {
        context.textBlockId = messageId || (0, import_node_crypto.randomUUID)();
    ...
    case "assistant.reasoning_delta": {
      ...
      if (!context.reasoningStarted) {
        context.reasoningBlockId = reasoningId || (0, import_node_crypto.randomUUID)();
...
var CopilotLanguageModel = class {
  ...
  ensureClient() {
    if (!this.client) {
      this.client = new import_copilot_sdk.CopilotClient({
